...
export const MealCard: React.FC<MealCardProps> = ({ item, dateStr, isOrdered, onSelect }) => {
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden', position: 'relative' }}>
      {/* Image Header */}
      <div style={{ position: 'relative', height: '160px', backgroundColor: '#e5e7eb' }}>
        <img
      ...
      <div style={{ padding: '14px 16px' }}>
        <h3 style={{ fontSize: '16px', marginBottom: '6px', color: 'var(--brand-brown-dark)' }}>
          {item.name}
        </h3>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '10px' }}>
          ...
          {item.may_contain && item.may_contain.length > 0 && (
            <span className="badge" style={{ backgroundColor: '#f3f4f6', color: '#6b7280', fontSize: '10px' }}>
        ...
        >
          {isOrdered ? (
            <>
              <Check size={18} /> Meal Pre-selected (£{item.price.toFixed(2)})
            </>
          ) : conflict.hasConflict ? (
            <>
