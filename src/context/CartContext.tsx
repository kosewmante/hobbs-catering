...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getCartItemForDateAndStudent = (date: string, studentId: string) => {
    return cartItems.find(item => item.date === date && item.student_id === studentId);
  };

  const isDateOrderedForStudent = (date: string, studentId: string) => {
    return orders.some(o => o.items.some(i => i.date === date && i.student_id === studentId));
  };

  const addItemToCart = (date: string, menuItem: MenuItem, student: Student) => {
    const conflict = checkDietaryConflict(menuItem, student);

    const existingIndex = cartItems.findIndex(i => i.date === date && i.student_id === student.id);

    const newItem: CartItem = {
      id: `cart-${student.id}-${date}-${menuItem.id}`,
      student_id: student.id,
      student_name: student.name,
      date,
      menu_item: menuItem,
      price: menuItem.price || 2.50,
  ...
  const acknowledgeConflict = (cartItemId: string) => {
    setCartItems(prev =>
      prev.map(item => (item.id === cartItemId ? { ...item, warning_acknowledged: true } : item))
    );
  };
